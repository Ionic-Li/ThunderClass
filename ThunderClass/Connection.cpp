...
【更改记录】
	...
	2020-05-15 由范静涛增加注释
	2020/6/17 由李永健增加快速发送下课消息的功能
	2020/6/18 由李永健增加快速发送题目消息功能
*************************************************************************/
bool Connection::Send(const Message& aMessage) {
	...
	if (m_bIsConnected) {
		...
		m_SendMsgMutex.lock();
		if (aMessage.Type() == MSG_CLASSEND || aMessage.Type() == MSG_QUESTION || aMessage.Type() == MSG_ENDQUES)//如果是退出或者是题目消息，就将原消息队列清空
		{
			m_ToSend = queue<Message>();
		}
