...
const unsigned int AudioFrame::BUF_SIZE = 1024 * 10;

//类静态成员初始化，表示录音和播放的音频格式
//请注意，这种结构体作为静态成员的初始化形式比较少见，初始化要加.成员名
const WAVEFORMATEX AudioFrame::FORMAT = {
    WAVE_FORMAT_PCM, //FORMAT.wFormatTag, 声音格式为PCM
    1,                //FORMAT.nChannels, 采样声道数，1声道
    8000,        //FORMAT.nSamplesPerSec, 采样率，8000次/秒
    8000,       //FORMAT.nAvgBytesPerSec, 每秒的数据率，就是每秒能采集多少字节的数据
    1,              //FORMAT.nBlockAlign, 一个块的对齐字节数，采样bit的字节数乘以声道数
    8,           //FORMAT.wBitsPerSample, 采样比特，16bits/次
    0                    //FORMAT.cbSize, 一般为0
};
...
AudioFrame::AudioFrame(): pBuffer(m_pBuffer), Header(m_Header) {
    ...
    m_Header.dwBufferLength = BUF_SIZE;
