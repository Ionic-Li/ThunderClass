...
【接口说明】
	...
	静态函数GetUser(int iNum)按照给定位置搜索用户
...
public:
	...
	//静态函数GetUser(int iNum)按照给定位置搜索用户
	static const User* GetUser(int iNum);
    //静态函数GetLoginedUser获得最后一次登录的用户指针
...
protected:
	...
	virtual bool DeleteUser(int iNum);
	virtual bool ChangeUser(int iNum, const string& Name, const string& Password, const string& Type);
	virtual string GetPassword(int iNum);
	virtual int SearchUser(const string& Name);
private:
    //保存到文件流
