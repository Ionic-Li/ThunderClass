...
bool Audio::GetAndRemoveFirstRecorded(AudioFrame& Frame) {
    ...
    if (!m_RecordedFrames.empty()) {
		//如果录音时间和当前时间相差3秒以上，就扔掉不要，避免延迟过于严重
		while (clock() - m_RecordedTimes.front() > 3000)
		{
