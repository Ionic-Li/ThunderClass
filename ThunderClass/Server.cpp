...
【更改记录】       无
*************************************************************************/
void Server::SendToExcept(unsigned int idx, const Message& aMessage)
{
	for (unsigned int i = 0; i < m_Connections.size(); i++)
	{
		if (i == idx)
		{
			continue;//跳过编号为idx的Connection
		}
		SendTo(i, aMessage);
	}
}
...
void Server::Close() {
	...
	SendToAll(Message(MSG_CLASSEND, nullptr, 0));
	//改变为非监听状态
	m_bIsListening = false;
	time_t lBegin = clock();//开始等待消息的时间
	while (1)//等待客户端发完注意力消息自主退出
	{
		int iCount = 0;//记录在线的端口数
		...
		if (0 == iCount)//如果没有在线的端口就退出
		{
			break;
		}
		if (clock() - lBegin >= 5000)//如果等待时间超过5秒就退出
		{
			break;
		}
	}
