...
【更改记录】
	...
	2020/6/19 由李永健增加返回待发队列数量的函数
	2020/6/21 由李永健增加清空待发队列的函数
*************************************************************************/
...
【参数】           无
【返回值】         无
【开发者及日期】   李永健 2020/6/21
【更改记录】       无
*************************************************************************/
void Client::ClearToSend()
{
	m_SendMsgMutex.lock();
	m_ToSend = queue<Message>();
	m_SendMsgMutex.unlock();
...
【更改记录】       无
*************************************************************************/
int Client::GetToSendCount() const
{
...
DWORD WINAPI Client::CommunicationThread(LPVOID lpParameter) {
	...
	Clnt->m_SendMsgMutex.lock();
	if (!Clnt->m_ToSend.empty())//避免专注信息没有被发出去的情况
	{
		SendCount = send(Clnt->m_SocketServer, (const char*)Clnt->m_ToSend.front().Data, Clnt->m_ToSend.front().Size, 0);
		Clnt->m_ToSend.pop();
	}
