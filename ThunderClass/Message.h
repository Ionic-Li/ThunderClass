...
【接口说明】
	...
	静态成员函数FromTimeCount产生一个时间统计的消息
	静态成员函数FromQuestion产生一个问题消息
	静态成员函数FromAnswer产生一个答案消息
	...
	静态成员函数ToToTimeCount从信息统计消息转化为信息统计对象
	静态成员函数ToQuestion从问题消息转化为问题对象
	静态成员函数ToAnswer从答案消息转化为答案对象
...
public:
	...
	static Message FromTimeCount(const TimeCount& aTimeCount);
	static Message FromQuestion(const Question& aQuestion);
	static Message FromAnswer(const Answer& anAnswer);
	...
	static TimeCount ToTimeCount(const Message& Msg);
	static Question ToQuestion(const Message& Msg);
	static Answer ToAnswer(const Message& Msg);
